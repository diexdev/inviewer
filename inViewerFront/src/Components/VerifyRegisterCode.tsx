...
import { verifyEmailCodeApi } from "../consts"
...
export function VerifyRegisterCode () {
  ...
  const handleVerify = async (e:React.FormEvent<HTMLFormElement>) => {
    ...
    try {
        const res = await fetch(verifyEmailCodeApi, {
        method: 'POST',
        headers: {
    ...
    <>
      <form className="verify-form" onSubmit={handleVerify}>
        ...
        <div className="send-options">
          <button onClick={() => navigate('/receiveEmailCode')} type="button">Back</button>
          <button onClick={() => navigate('/register')} disabled={sent.current === false} type="button">Next</button>
        </div>
        <Link to={'/'}>Log In</Link>
      </form>
    </>
